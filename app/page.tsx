...
import { Target, MessageSquareOff, Layout, Lightbulb, ArrowRight, CheckCircle2, Star, Zap, Globe, Users, TrendingUp } from "lucide-react";
    ...
    <main>
        ...
        <div style={{ ...maxW, paddingTop:"7rem", paddingBottom:"5rem", position:"relative", zIndex:1, width:"100%" }}>
          <div style={{ maxWidth:740 }}>
            ...
            <p className="hero-sub" style={{ fontSize:"clamp(1rem,2vw,1.2rem)", color:"rgba(255,255,255,0.7)", lineHeight:1.7, margin:"0 0 2.5rem", maxWidth:580 }}>
              I help founders and businesses in Bangladesh build brands that actually convert — through positioning, offer design, and strategic clarity. Not more ads. Not more content. The right strategy first.
            </p>
            <div className="hero-cta" style={{ display:"flex", flexWrap:"wrap", gap:"1rem", alignItems:"center" }}>
              <Link href="/consult" style={{ display:"inline-flex", alignItems:"center", gap:"0.5rem", padding:"1rem 2.25rem", background:acc, color:"white", borderRadius:9999, fontWeight:700, fontSize:"1rem", textDecoration:"none" }}>Book a Free Strategy Diagnostic <ArrowRight size={18} /></Link>
              <Link href="/projects" style={{ display:"inline-flex", alignItems:"center", gap:"0.5rem", padding:"1rem 2rem", background:"transparent", color:"rgba(255,255,255,0.8)", border:"1.5px solid rgba(255,255,255,0.25)", borderRadius:9999, fontWeight:600, fontSize:"0.95rem", textDecoration:"none" }}>View My Work</Link>
            </div>
      ...
      <section style={{ background:"hsl(270 25% 95%)", padding:"6rem 1.5rem" }}>
        <div style={maxW}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:"1.5rem" }}>
            {[
              { quote:"Hasib completely reframed how we think about our positioning. Before working with him, we were running ads with no clear message. After his strategy session, we knew exactly who we were and who we were for.", name:"Rafiq Ahmed", role:"Founder, E-commerce Brand", stars:5 },
              { quote:"The Brand Blueprint session was worth ten times what I paid. He didn't just give me tactics — he showed me why everything I'd been doing wasn't connecting, and what to do instead.", name:"Nadia Islam", role:"CEO, Service Business", stars:5 },
              { quote:"I've hired digital marketers before and always felt like they were guessing. Working with Hasib felt like working with someone who understood what was wrong and had a clear framework to fix it.", name:"Tanvir Hossain", role:"Founder, Tech Startup", stars:5 },
              ...
              <ScrollReveal key={name} delay={i*120}>
                <div style={{ background:"white", border:"1px solid hsl(270 20% 88%)", borderRadius:"1.25rem", padding:"2rem", display:"flex", flexDirection:"column", gap:"1.25rem" }}>
                  <div style={{ display:"flex", gap:"0.25rem" }}>{Array.from({length:stars}).map((_,si)=><Star key={si} size={16} fill="hsl(275 70% 55%)" style={{ color:"hsl(275 70% 55%)" }} />)}</div>
                  <p style={{ fontSize:"0.925rem", color:"hsl(270 15% 35%)", lineHeight:1.75, margin:0, fontStyle:"italic" }}>&ldquo;{quote}&rdquo;</p>
                  <div><div style={{ fontWeight:700, fontSize:"0.9rem", color:"hsl(270 20% 15%)" }}>{name}</div><div style={{ fontSize:"0.8rem", color:"hsl(270 15% 55%)", marginTop:"0.1rem" }}>{role}</div></div>
                </div>
              </ScrollReveal>
            ))}
          </div>
