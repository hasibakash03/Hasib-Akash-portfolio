...
export const metadata: Metadata = {
  title: "Blog",
  description: "Brand strategy, positioning frameworks, and growth insights applied to the Bangladeshi market. No generic Western templates.",
  openGraph: {
    title: "Blog — Hasib Akash",
    description: "Strategic thinking for Bangladeshi founders. Brand strategy, positioning, and growth frameworks.",
    images: [{ url: "/og?title=Strategic+Thinking+for+Bangladeshi+Founders&type=blog", width: 1200, height: 630 }],
  },
};
...
export default async function BlogPage() {
  ...
  const posts = dbPosts
    ? dbPosts.map(p => ({ slug: p.slug, title: p.title, category: p.category, date: p.publishedAt ? new Date(p.publishedAt).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) : "", readTime: p.readTime, featured: false, thumbnail: p.featuredImageUrl || "hsl(270 55% 88%)", excerpt: p.excerpt, tags: Array.isArray(p.tags) ? p.tags as string[] : [] }))
    : staticPosts;
