...
export default function OfferCards() {
  ...
  const [activeCard, setActiveCard] = useState(2); // default: Card 3

  /* ── Carousel: scroll to Card 3 on mobile mount + live dot tracking ── */
  useEffect(() => {
    const el = carouselRef.current;
    if (!el) return;

    // Scroll to Card 3 (index 2) on initial mobile load
    const initialScroll = () => {
      if (window.innerWidth >= 768) return;
      const cards = el.querySelectorAll<HTMLElement>(".offer-card");
      if (cards[2]) {
        el.scrollLeft = cards[2].offsetLeft - el.offsetLeft;
      }
    };
    // Run after paint so offsetLeft is available
    ...
    <>
      ...
      <style>{`
        ...
        .offers-grid {
          ...
          /* let badge overflow without being clipped */
          overflow-y: visible;
          clip-path: none;
        }
        ...
        /* Neutralise the desktop elevation transform on mobile/tablet */
        .offer-card-featured { transform: none !important; }

        /* CTA always pinned to card bottom */
        .offer-cta { margin-top: auto !important; }
        ...
        /* ── Tablet: 2 × 2 grid, Card 3 top-right ── */
        @media (min-width: 768px) {
          ...
          /* Reorder: 1 top-left, 3 top-right, 2 bottom-left, 4 bottom-right */
          .offer-card:nth-child(1) { order: 1; }
          .offer-card:nth-child(2) { order: 3; }
          .offer-card:nth-child(3) { order: 2; }
          .offer-card:nth-child(4) { order: 4; }

          .offers-dots { display: none; }
        }

        /* ── Desktop: single row of 4, equal height, buttons pinned ── */
        @media (min-width: 1024px) {
          .offers-grid {
            grid-template-columns: repeat(4, 1fr);
            align-items: stretch;
          }
          /* Restore natural DOM order */
          .offer-card:nth-child(1) { order: 1; }
          .offer-card:nth-child(2) { order: 2; }
          .offer-card:nth-child(3) { order: 3; }
          .offer-card:nth-child(4) { order: 4; }
          /* Restore elevation on Card 3 for desktop */
          .offer-card-featured { transform: translateY(-6px) !important; }
        }
      `}</style>

      {/* ── Cards grid / carousel ──────────────────────────────────── */}
      <div ref={carouselRef} className="offers-grid">

        {/* ── Card 1 — FREE / Cold Client Audit (no badge) ─────────── */}
        <div className="offer-card" style={{
          background: "white", border: "1px solid hsl(270 20% 88%)",
          borderRadius: "1.5rem", padding: "2.25rem", position: "relative",
        }}>
          {/* header */}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", marginBottom: "1.75rem" }}>
            {["30-minute 1-on-1 strategy call","Cold client gap diagnosis — specific to your business","1-page written summary delivered within 24 hours","No pitch, no pressure, no obligation"].map(item => (
          ...
          <a className="offer-cta" href="#apply"
            style={{ display: "block", textAlign: "center", padding: "0.875rem", background: "hsl(270 60% 96%)", color: "hsl(275 70% 45%)", borderRadius: 9999, fontWeight: 700, fontSize: "0.9rem", textDecoration: "none", border: "1px solid hsl(270 30% 85%)", transition: "all 0.2s" }}
        ...
        <div className="offer-card" style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", marginBottom: "1.5rem" }}>
            {["90-minute live strategy session","Cold client positioning analysis","Target audience mapping","Competitive differentiation framework","Written Brand Blueprint PDF delivered after","Session recording for your reference"].map(item => (
              <div key={item} style={{ display: "flex", gap: "0.6rem", alignItems: "flex-start" }}>
                <CheckCircle2 size={15} style={{ color: acc, flexShrink: 0, marginTop: 1 }} />
                <span style={{ fontSize: "0.875rem", color: "hsl(270 15% 40%)" }}>{item}</span>
              </div>
        ...
        {/* ── Card 3 — 19,900৳ / Cold Client Sprint (MOST POPULAR) ─── */}
        <div className="offer-card offer-card-featured" style={{
          background: "linear-gradient(135deg,hsl(270 60% 14%) 0%,hsl(275 55% 28%) 100%)",
          borderRadius: "1.5rem", padding: "2.5rem 2.25rem", position: "relative",
          boxShadow: "0 32px 80px hsl(270 60% 30%/0.5)", zIndex: 1,
        }}>
          {/* badge */}
          ...
          <a className="offer-cta" href="#apply"
            style={{ display: "block", textAlign: "center", padding: "0.875rem", background: "hsl(275 70% 55%)", color: "white", borderRadius: 9999, fontWeight: 700, fontSize: "0.9rem", textDecoration: "none", transition: "all 0.2s" }}
      ...
      <div className="offers-dots" aria-hidden="true">
        {[0, 1, 2, 3].map(i => (
          <div key={i} className={`offers-dot${activeCard === i ? " offers-dot-active" : ""}`} />
        ))}
      </div>
